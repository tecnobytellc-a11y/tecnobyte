...
const AppContent = () => {
    ...
    const {
        ...
        reloadlyCatalog, setReloadlyCatalog,
        availableCountries, setAvailableCountries,
        rangoInputs, setRangoInputs,
        currentPage, setCurrentPage,
        activeUser, setActiveUser,
        isAuthLoading, setIsAuthLoading,
        cashbackPct, setCashbackPct,
        lastOrder, setLastOrder,
        exchangeRateBs, setExchangeRateBs,
        checkoutStep, setCheckoutStep,
        paymentMethod, setPaymentMethod,
        paypalData, setPaypalData,
        proofData, setProofData,
        isProcessing, setIsProcessing,
        isBlocked, setIsBlocked,
        showTerms, setShowTerms,
        showPrivacy, setShowPrivacy,
        coupon, setCoupon,
        isLoadingSecurity, setIsLoadingSecurity,
        isLoadingCatalog, setIsLoadingCatalog,
    ...
    // Eliminamos los useState locales que ahora están en el store
    ...
    useEffect(() => {
        const init = async () => {
            try {
                // 1. Verificación robusta en Backend en lugar de ipwho.is directo
                try {
                    const securityRes = await api.get('/auth/check-security');
                    if (securityRes.data?.blocked) {
                        setIsBlocked(true);
                        setIsLoadingSecurity(false);
                        return;
                    }
                } catch (secError) {
                    console.error("[SECURITY] Error al verificar IP:", secError);
                    // Fail-open
                }

                // 2. Extraer configuración desde Firebase Directamente
                const configDoc = await getDoc(doc(db, "configuracion", "global"));
                if (configDoc.exists()) {
                    const cfg = configDoc.data();
                    setServices(cfg.catalog || []);
                    setMultipackages(cfg.multipackages || {});
                    setContactInfo(cfg.contact || DEFAULT_CONTACT_INFO);
                    setLegalInfo(cfg.legal || { terms: 'Términos', privacy: 'Privacidad' });
                    setSocialLinks(cfg.social || {});
                } else {
                    console.warn("[CONFIG] No se encontró config global en Firestore");
                }

                // 3. Cargar Catálogo de Reloadly
                const reloadlySnapshot = await getDocs(collection(db, "catalogo_reloadly"));
                const catalog = {};
                const paises = new Set(['All']);

                reloadlySnapshot.forEach(docSnap => {
                    const paquetes = docSnap.data().paquetes;
                    catalog[docSnap.id] = paquetes;
                ...
                console.error("[CRITICAL] Fallo en inicialización core:", error);
            } finally { 
                setIsLoadingSecurity(false); 
    ...
    useEffect(() => { 
        ...
        const img = new Image(); 
        img.src = '/unnamed.png'; 
        img.onload = () => { link.href = '/unnamed.png'; }; 
    }, []);
    ...
    useEffect(() => { 
        ...
        const fetchRate = async () => { 
            ...
            try { 
                ...
                if (!response.ok) throw new Error('Error tasa HTTP ' + response.status); 
                ...
                console.error("[RATE] Fallo al actualizar la tasa:", error);
            } finally {
                isFetching = false;
            }
        }; 
    ...
    // Usamos las funciones del store directamente
    ...
    const solicitarPinAutomatico = async (idProducto, valorTarjeta, precioPagado, correoCliente) => {
        try {
            const respuesta = await api.post('/reloadly/comprar-pin', {
            ...
            <main className="flex-grow pt-6 pb-20">
                <Routes>
                    ...
                    <Route path="/perfil" element={
                        <ProtectedRoute isAuthLoading={isAuthLoading} activeUser={activeUser}>
