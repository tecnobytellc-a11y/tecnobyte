    ...
    <div className="bg-[#0a0a12] text-gray-100 min-h-screen font-sans">
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="flex-1 space-y-6 overflow-y-auto pr-2">
              {cart.length === 0 ? (
                <div className="text-center py-20 text-gray-500">
                  ...
                  <p className="uppercase font-bold text-xs tracking-widest">Tu carrito está vacío</p>
                </div>
              ) : (
                cart.map((item, idx) => (
                  <div key={idx} className="bg-gray-800/50 p-4 rounded-xl border border-white/5">
                    <div className="flex justify-between items-center">
                      <div>
                        <h4 className="text-white font-medium">{item.title}</h4>
                        <p className="text-sm text-indigo-400 font-bold">${item.price.toFixed(2)}</p>
                      </div>
                      <button 
          ...
          {/* Cierres adicionales para asegurar la estructura del componente padre */}
          </div>
        </div>
      )}
