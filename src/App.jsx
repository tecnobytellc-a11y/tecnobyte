...
const Navbar = ({ cartCount, onOpenCart, setView }) => ( <nav className="fixed top-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-indigo-500/30 shadow-[0_0_15px_rgba(79,70,229,0.3)]"><div className="max-w-7xl mx-auto px-4"><div className="flex items-center justify-between h-20"><div className="relative flex items-center cursor-pointer h-20 w-48 sm:w-64 -ml-8 sm:-ml-12" onClick={() => setView('home')}><img src="logo.png" alt="TecnoByte" className="w-full h-full object-contain scale-[1.4] sm:scale-[1.6] origin-left" onError={(e) => { e.target.onerror = null; e.target.src = "unnamed.png"; }} /></div><div className="relative group cursor-pointer" onClick={onOpenCart}><ShoppingCart className="w-7 h-7 text-gray-300 group-hover:text-cyan-400 transition-colors" />{cartCount > 0 && <span className="absolute -top-2 -right-2 bg-pink-600 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">{cartCount}</span>}</div></div></div></nav> );
