...
import PayPalCardProcessorEUR from './components/checkout/PayPalCardProcessorEUR';
...
const AppContent = () => {
    ...
    const handleCheckoutStart = () => { 
        ...
        const hasCardTestItem = cart.some(i => i.isCardTest);
        setTimeout(() => { 
            if (hasCardTestItem) { setPaymentMethod('tarjeta_eur'); setCheckoutStep(1); }
            else if (hasExchangeItem) { setPaymentMethod('paypal'); setCheckoutStep(1); } 
                ...
                <Routes>
                    ...
                    <Route path="/checkout" element={
                        <div className="pt-24 px-4 sm:px-6 lg:px-8">
                            ...
                            {checkoutStep === 1 && (paymentMethod === 'paypal' || paymentMethod === 'binance' || paymentMethod === 'tarjeta' || paymentMethod === 'tarjeta_eur' || paymentMethod === 'saldo_tnb') && <PayPalDetailsForm paypalData={paypalData} setPaypalData={setPaypalData} setCheckoutStep={setCheckoutStep} paymentMethod={paymentMethod} openTerms={() => setShowTerms(true)} openPrivacy={() => setShowPrivacy(true)} cart={cart} />}
                            {checkoutStep === 2 && ( 
                                (paymentMethod === 'tarjeta_eur') ? <PayPalCardProcessorEUR cart={cart} finalTotal={finalTotal} coupon={coupon} paypalData={paypalData} setLastOrder={setLastOrder} setCart={setCart} setCheckoutStep={setCheckoutStep} /> :
                    ...
                    <Route path="/card-test" element={
                        <CardTestPage addToCart={addToCart} setIsCartOpen={setIsCartOpen} />
                    } />
