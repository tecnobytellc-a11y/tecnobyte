...
const AppContent = () => {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                const ipRes = await fetch('https://ipwho.is/'); // IPWHO es externo, mantenemos fetch o usamos api sin baseURL si fuera necesario, pero fetch está bien para servicios de terceros.
                if(ipRes.ok) {
                    ...
                    if (ipData.success) {
                        ...
                        if (["vpn", "proxy", "hosting", "cloud", "datacenter"].some(k => org.includes(k) || asn.includes(k))) { 
                            setIsBlocked(true); 
                        ...
                        try { 
                            const checkRes = await api.get(`/admin/check-ip?ip=${ipData.ip}`); 
                ...
                const configRes = await api.get('/admin/get-config');
                const config = configRes.data;
                if (config.success) { 
                    setServices(config.catalog); 
                    setMultipackages(config.multipackages || {}); 
                    setContactInfo(config.contact); 
                    setLegalInfo(config.legal); 
                    setSocialLinks(config.social); 
                }
