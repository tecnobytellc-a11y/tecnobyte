...
const ProductCard = ({ service, addToCart, exchangeRateBs, idx, multipackages }) => {
  ...
  const handleAdd = () => {
    if (service.isTnbRecharge) {
      ...
      addToCart({
        ...service,
        title: `${service.title} de ${service.isEuros ? '€' : '$'}${customAmount}`, 
        faceValue: Number(customAmount),               
        price: Number(precioFinalCalculado.toFixed(2)) 
      });
  ...
  return (
    <motion.div 
        ...
        whileHover={{ y: -8, boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.05)" }}
        className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-indigo-300 transition-all duration-300 group shadow-sm flex flex-col justify-between"
    >
      <div>
        {customIconUrl ? (
        ...
        {service.isCustomAmount && !service.isTnbRecharge && (
            <div className="mt-4 p-5 bg-slate-50 rounded-xl border border-slate-200">
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">
                    Monto ({service.isEuros ? '€' : '$'}{service.minAmount} - {service.isEuros ? '€' : '$'}{service.maxAmount})
                </label>
                <div className="flex items-center gap-3">
                    <span className="text-2xl text-slate-400 font-bold">{service.isEuros ? '€' : '$'}</span>
                    <input 
                        type="number" 
                ...
                <div className="mt-3 flex justify-between items-center text-sm">
                    ...
                    <span className="text-indigo-600 font-extrabold">{service.isEuros ? '€' : '$'}{precioFinalCalculado.toFixed(2)} {service.isEuros ? 'EUR' : 'USD'}</span>
                </div>
            </div>
        )}
      ...
      <div className="flex items-center justify-between mt-auto pt-5 border-t border-slate-100">
        <div className="flex flex-col">
          <span className="text-2xl font-black text-slate-800">
            {service.isEuros ? '€' : '$'}{currentPrice.toFixed(2)}
          </span>
