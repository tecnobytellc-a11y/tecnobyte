...
const GamificationDashboard = () => {
    ...
    useEffect(() => {
        const reclamarCajaMensual = async () => {
            ...
            if (auth.currentUser && userData) {
                try {
                    const response = await api.post('/gamification/check-monthly-box', {
    ...
    const handleStart2FASetup = async (method) => {
        if (method === 'passkey') {
            ...
            try {
                ...
                const resOpts = await api.post('/auth/2fa-passkey-register-start', {
                    userId: auth.currentUser.uid,
                ...
                const resFinish = await api.post('/auth/2fa-passkey-register-finish', {
                    userId: auth.currentUser.uid,
                    response: attResp,
                    isSetup: true
                });
    ...
    const handleVerify2FACode = async (e) => {
        ...
        try {
            ...
            if (twoFAMethod === 'email') {
                ...
                res = await api.post('/auth/2fa-verify', { codigo: authCode, secret: tempSecret });
            }
    ...
    const handleRedeemGiftCard = async () => {
        ...
        setIsRedeeming(true);
        try {
            const response = await api.post('/gamification/redeem-giftcard', {
                userId: auth.currentUser.uid,
                code: giftCode
            });
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
                <h1 className="text-4xl md:text-6xl font-black font-orbitron text-gradient uppercase tracking-widest drop-shadow-sm">
                    Mi Perfil Gamer
                </h1>
